...
# creating new clean object drought_exp_clean from drought_exp data frame
drought_exp_clean <- drought_exp |> 
  # removing underscores and lowering
  clean_names() |> 
  # mutating names to make them properly labeled
  ...
  # expanding out water treatment names
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  ))  |> 
  # setting the levels to be the different native plants with first level being "Lotus scorparius" -- reordering species levels
  ...
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |> 
  # selecting the columns of interest: species, water treatment, specific leaf area, and total plant mass
...
ggplot(
  ...
  labs(
    ...
    title = "Specific leaf area and total mass (g) increase together"
  )
...
ggplot(
  ...
  labs(
    ...
    y = "Total mass (g)",
    # to add line break: \n, so \nhave
    title = "Well watered plants have a greater median mass"
  ) +
...
# base layer ggplot
ggplot(
  # data frame is cleaned drought data
  data = drought_exp_clean,
  mapping = aes(
    x = species_name, # x axis species name
    y = total_g, # y axis total mass in g
    color = species_name # color by species name
  )
) +
  # first layer jitter
  geom_jitter(
    # doesnt allow points to move up and down y axis
    height = 0,
    # keeps points together
    width = 0.2
  ) +
  ...
  labs(
    x = "Species",
    y = "Total mass (g)",
    title = "Grindelia camporum and eschscholzia have \nhighest mean total mass "
  ) +
  # get rid of legend and make the little labels angeled, hjust = 1 makes them not overlap the axes line
  theme(
...
- residuals appear homoscendastic (have constant variances)
...
- species_namePenstemon centranthifolius: model prediction for the different between Pensetom mass in the well watered treatment and Lotus mass in the well watered treatment
- species_nameEschscolzia californica: model prediction for the difference between Escholzia mass in the well watered treatment and Lotus mass in the well watered treatment
